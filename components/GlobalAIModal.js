...
import { FaImage, FaVideo, FaTimes, FaMagic, FaSpinner, FaSave, FaTrash, FaDownload, FaRobot, FaCheck } from 'react-icons/fa';
...
export default function GlobalAIModal() {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `ai-${generationType}-${Date.now()}.${generationType === 'video' ? 'mp4' : 'png'}`;
  ...
  const handleDiscard = () => {
    if (isGenerating) {
      if (!window.confirm('Generation is in progress. Are you sure you want to discard?')) {
        return;
      }
    }
    resetGeneration();
        ...
        <div className="p-4 space-y-4 max-h-[70vh] overflow-y-auto">
          ...
          {result && !saveSuccess && (
            <div className="space-y-4">
              <p className="text-white/40 text-xs">Generated {generationType === 'video' ? 'Video' : 'Image'}</p>
